...
- [ ] 1.1 Promote `time` to a direct dependency in `Cargo.toml` as
...
- [ ] 2.1 Add failing tests in `tests/epub.rs`:
...
- [ ] 2.3 Replace the `mime_guess::get_mime_extensions_str(..).first()`
...
- [ ] 2.4 Confirm the pre-existing fallback tests at `tests/epub.rs:62-76`
...
- [ ] 5.1 Update the `epub/` section of `AGENTS.md`: `content_opf` now takes
...
- [ ] 6.1 `cargo test`
- [ ] 6.2 `cargo clippy --all-targets` with zero warnings
- [ ] 6.3 `cargo fmt --check`
- [ ] 6.4 Rebuild one real book with `--epub-only` and confirm `epubcheck`
