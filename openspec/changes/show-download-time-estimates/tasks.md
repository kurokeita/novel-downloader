...
- [ ] 1.1 Add failing tests in `tests/ui.rs` for elapsed time: reported before any
...
- [ ] 1.2 Add `started_at: Instant` and `finished_at: Option<Instant>` to
...
- [ ] 1.3 Add `finish_at(now)` and make the existing `finish()` delegate to it with
...
- [ ] 2.1 Add failing tests in `tests/ui.rs` for `HH:MM:SS` rendering: four minutes
  twelve seconds renders `00:04:12`, one hour nine minutes six seconds renders
  `01:09:06`, and zero renders `00:00:00`.
- [ ] 2.2 Implement the unconditional `HH:MM:SS` formatter in
  `src/ui/widgets/progress.rs` and export it from `ui::widgets`.

## 3. The completion window

- [ ] 3.1 Add failing tests in `tests/ui.rs` for window recording: a terminal event
...
- [ ] 3.2 Add `completions: VecDeque<Instant>` with a `MAX_RATE_SAMPLES` cap of 20,
  and `record_completed_at(number, status, now)` /
  `record_failed_at(number, message, now)` that push-back and pop-front.
- [ ] 3.3 Make `record_completed` / `record_failed` and `from_event` delegate to the
...
- [ ] 4.1 Add failing tests in `tests/ui.rs` for every case where the estimate is
...
- [ ] 4.2 Add failing tests in `tests/ui.rs` for the cases where an estimate is
...
- [ ] 4.3 Implement `eta(now) -> Option<Duration>` with a `MIN_RATE_SPAN` of two
...
- [ ] 5.1 Add failing tests in `tests/ui.rs` for the gauge label in its three
...
- [ ] 5.2 Implement a pure `gauge_label(&DownloadProgress, now) -> String` in
  `src/ui/widgets/progress.rs` composing the existing tally and percentage with
  the elapsed time and the estimate.
- [ ] 5.3 Replace the inline label construction in `draw_download`
...
- [ ] 6.1 Update the `ui/` and `Useful project context` sections of `AGENTS.md` to
  ...
  guessed.
- [ ] 6.2 Confirm every function added in tasks 1-5 carries a `///` doc comment,
  including the two tuning constants.

## 7. Verification

- [ ] 7.1 `cargo test` green.
- [ ] 7.2 `cargo clippy --all-targets` reports 0 warnings.
- [ ] 7.3 `cargo fmt --check` clean.
- [ ] 7.4 Eyes-on-terminal check against a local mock site: run the TUI through a
