...
- [ ] 1.1 Add failing tests in `tests/utils.rs` for a `utils` function that
      answers "can this path be read as a font", covering a real TTF fixture, a
      missing path, and a non-font file
- [ ] 1.2 Implement that function next to `find_font_file` in `src/utils.rs`,
      returning the canonical path plus the extracted `FontMetadata` on success
      and an error otherwise

## 2. Store scaffolding

- [ ] 2.1 Add failing tests in a new `tests/recent_fonts.rs` for load/record
...
- [ ] 2.2 Create `src/recent_fonts.rs` with the `RecentFont` entry, the serde
      container carrying `#[serde(default)]`, `load(config_dir)`,
      `record(config_dir, path)`, and export the module from `src/lib.rs`
- [ ] 2.3 Add failing tests for the tolerant-read behavior: a missing file, an
      ...
      store
- [ ] 2.4 Implement the tolerant read so any parse or read failure yields an
      empty list

## 3. Ordering and capacity

- [ ] 3.1 Add failing tests for ordering: recording an already-remembered font
...
- [ ] 4.3 Add failing tests for stale-metadata refresh: an unchanged font is
      ...
      keeps its position
- [ ] 4.4 Implement the size-mismatch refresh, re-parsing only the mismatched
      entry

## 5. Recording rules

- [ ] 5.1 Add failing tests for the record guards: a path that cannot be read as
      a font leaves the store unchanged, and the bundled font's own path is never
      stored
- [ ] 5.2 Implement both guards in `record`, comparing the canonical path against
      the canonical path returned by `find_font_file(None)` for the bundled-font
      check
- [ ] 5.3 Add failing tests for the environment resolver: `$XDG_CONFIG_HOME`
      wins when set and non-empty, `$HOME/.config` is the fallback, and neither
      set yields `None`
- [ ] 5.4 Implement the `config_dir()` resolver, keeping it the only part of the
      module that reads the environment

## 6. Wizard integration

- [ ] 6.1 Extend `FontChoice` in `src/ui/wizard/state.rs` with a
      `Remembered(PathBuf)` variant and add the cached
      `Option<Vec<RecentFont>>` field to `WizardState`
- [ ] 6.2 Make `step_font_choice` async in `src/ui/wizard/steps.rs`, populate the
...
- [ ] 6.3 Route a `Remembered` selection straight to `WizardStep::Confirm` with
...
- [ ] 6.4 Make `step_font_path` validate the submitted path with the task-1
      primitive, showing a note and re-entering `WizardStep::FontPath` on failure
- [ ] 6.5 Record the confirmed font in `run_interactive_flow` after
      `StepResult::Done`, reading `plan.font_path` and skipping when it is `None`
      or when `config_dir()` is `None`

## 7. Documentation

- [ ] 7.1 Add `recent_fonts` to the `AGENTS.md` module list and record the store
      location, cap, and prune-on-load behavior in its useful-context section

## 8. Verification

- [ ] 8.1 `cargo test` green
- [ ] 8.2 `cargo clippy --all-targets` with 0 warnings
- [ ] 8.3 `cargo fmt --check` clean
- [ ] 8.4 Eyes-on-terminal check of the font step: empty list, a populated list,
