...
- [ ] 1.1 Record `tests/fixtures/xtruyen_novel.html` from the reference novel
      ...
      placeholder prose
- [ ] 1.2 Record `tests/fixtures/xtruyen_chapter.html` keeping the chapter
...
- [ ] 1.3 Record `tests/fixtures/xtruyen_chapter_suffixed.html` for a window
      containing suffixed chapter addresses, which is the case the index walk
      exists for
- [ ] 1.4 Note in a comment beside the fixtures how the encoded payload was
      produced, so a future edit can regenerate it

## 2. Payload decoding

- [ ] 2.1 Add failing tests in `tests/source.rs` for the decoder: a payload
...
- [ ] 4.3 Add a failing test asserting sequence numbers run from 1 with no
      gaps, and that the locator is the chapter's own address
- [ ] 4.4 Add a failing test asserting a page that cannot be retrieved
      mid-walk fails the whole index rather than returning a short novel
- [ ] 4.5 Add a failing test asserting the walk terminates when a window
      yields no new address, so a site that links a chapter back to itself
      cannot loop forever
- [ ] 4.6 Add the `///`-documented walk in `src/source/xtruyen/discovery.rs`

## 5. The adapter and its registration

- [ ] 5.1 Add failing tests for URL acceptance: a novel page URL with and
...
- [ ] 5.2 Add a failing test asserting `fetch_metadata` returns metadata
      without requesting the chapter index, which is what `--epub-only`
      relies on
- [ ] 5.3 Add a failing test asserting a `429` response surfaces as the
      rate-limited error variant rather than a generic failure
- [ ] 5.4 Add a failing test asserting a chapter page with no chapter label
      still yields a chapter titled from its sequence number
- [ ] 5.5 Add `src/source/xtruyen/mod.rs` with the `///`-documented
      `SiteAdapter` impl, deriving every request base from the URL it was
      handed rather than a compiled-in host
- [ ] 5.6 Add the entry to `ADAPTERS` in `src/source/registry.rs`
- [ ] 5.7 Add a failing test asserting the new host appears in the
      unsupported-host error message, since that list is generated and a
      missing entry means the adapter is not really wired in
- [ ] 5.8 Write `rate_policy` with concurrency 2, a 500ms minimum delay, a 2s
...
- [ ] 6.1 Add a failing inline `#[cfg(test)]` test in `src/source/mod.rs` for
...
- [ ] 6.2 Add the `///`-documented predicate that the test drives
- [ ] 6.3 Add a failing inline test in `src/ui/wizard/state.rs` for the step
      that follows the end-chapter prompt: fixed pacing yields the
      existing-file prompt, free pacing yields the worker prompt
- [ ] 6.4 Add the `///`-documented pure transition function alongside
      `step_after_mode`, keeping it and `WizardStep` crate-internal
- [ ] 6.5 Cache the resolved pacing on `WizardState` once the URL is
      accepted, following the `recent_fonts` pattern so back-navigation does
      not re-resolve it
- [ ] 6.6 Route `step_end_chapter` through the new function, and change the
      back target of `step_if_exists` to the end-chapter prompt when the
      pacing prompts were skipped
- [ ] 6.7 Show the note stating the enforced worker count and delay, and that
      the site requires them, on entry to the locked path
- [ ] 6.8 Confirm the wizard writes the enforced values into the plan, so a
      locked run cannot carry a stale requested value into the runner

## 7. Reporting the pacing that is in force

- [ ] 7.1 Add failing tests in `tests/ui.rs` for `build_summary`: a fixed
      pacing source reports the source's worker count and delay, an
      unconstrained one reports the user's
- [ ] 7.2 Change `SummaryParams` and `build_summary` accordingly and let the
      compiler push the call sites, since the project keeps no compatibility
      shims
- [ ] 7.3 Add a failing test in `tests/cli.rs` or `tests/runner.rs` for the
      delay override notice, matching the existing worker-clamp event
- [ ] 7.4 Emit the delay override notice from the binary, and confirm no
      notice is emitted when the requested pacing is already within policy

## 8. Documentation

- [ ] 8.1 Add `xtruyen.vn` to the supported-hosts list in `README.md`, with a
      note that downloads from it are paced by the site and that the wizard
      therefore does not ask for workers or delay
- [ ] 8.2 Add the `source/xtruyen` bullet to the architecture section of
      ...
      sequence numbering
- [ ] 8.3 Update the `AGENTS.md` note on chapter URL conventions, which
      currently states that output files are numbered from the site's own
      chapter number

## 9. Verification

- [ ] 9.1 `git grep` for the new host and confirm no site-specific string
      lives outside `src/source/xtruyen/` and the registry entry
- [ ] 9.2 `cargo test`
- [ ] 9.3 `cargo clippy --all-targets`
- [ ] 9.4 `cargo fmt --check`
- [ ] 9.5 `cargo build --release`
