...
- **Lead line** — `**novel-downloader vX.Y.Z** — <feature / packaging / fix>. <one-sentence summary>. <breaking-change note, or "No breaking CLI changes from vPREV.">`
...
- **`## Install`** — Homebrew (`brew tap kurokeita/brew` + `brew install novel-downloader`) and winget (`winget install Kurokeita.NovelDownloader`) blocks
...
- Windows x86_64 (`novel-downloader-windows-x86_64.zip`)
