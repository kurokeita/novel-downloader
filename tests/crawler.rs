use novel_downloader::crawler::{build_html_document, escape_html};
use novel_downloader::source::SiteAdapter;
use novel_downloader::source::metruyenhot::{
    Metruyenhot, discover_last_chapter_number_from_html, extract_full_chapter_text,
};

/// Highest chapter number the adapter's index reaches for `base_url`, which
/// is what the old `discover_last_chapter_number` returned.
async fn discover_last_chapter_number(base_url: &str) -> anyhow::Result<u32> {
    let novel = Metruyenhot.fetch_novel(base_url).await?;
    novel
        .chapters
        .last()
        .map(|chapter| chapter.number)
        .ok_or_else(|| anyhow::anyhow!("empty chapter index for {base_url}"))
}
...
fn find_last_page_url_resolves_relative_href_against_main_url() {
    use novel_downloader::source::metruyenhot::find_last_page_url;
...
fn max_chapter_in_html_ignores_chuong_links_under_a_different_novel_slug() {
    ...
    use novel_downloader::source::metruyenhot::max_chapter_in_html;
    let html = r#"<html><body>
...
fn extract_full_chapter_text_handles_metruyenhot_chapter_fixture() {
    use novel_downloader::source::metruyenhot::extract_full_chapter_text;
...
fn discover_last_chapter_number_handles_metruyenhot_novel_fixture() {
    use novel_downloader::source::metruyenhot::discover_last_chapter_number_from_html;
